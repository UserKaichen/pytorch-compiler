...
def send_calcuaddrvar(fw, path, cnt):
    global fmain
    global logpath
    global layer_cnts
...
def test_chip_relocate84(layer_locate, outchl, C):
    ...
    for x in range(int(C/2)):  # outchannel/2组bn+weight
        ...
        for i in range(len(bn_locate)):
            if bn_locate[i] != '00':
                prints(f'test bnlocate[{i}] failed')
                return
...
def weight_addr(name_list, data_list):
    """
    description:
    ...
    fmain.write(f'weight_addr start:{ADDRBLOCK} bus_addr:{bus_address()} chiplet_num:{2 ** chiplet_num}\n')
...
def gen_ddraddr(name_list, data_list, netpath):
    fmain.write(f'dram_capacity:{dram_capacity}\n')
    netinout_addr(netpath)
    weight_addr(name_list, data_list)
    global CALCULATE
    CALCULATE = ADDRBLOCK
    otherinout_addr()
    prints("run gen_ddraddr successfully")
    return layers_act_addr, layers_wet_addr, datas_locate
...
def fc_CHW(layer_num):
    ...
    if layer_num == layer_cnts:
        C = int(line.split("in_features_y:")[1].split(" ")[0])
        Cout = int(line.split("out_features_y:")[1].split(" ")[0])
        return C, H, W, Cout
...
def pool_CHW(layer_num):
    ...
    global pool_fc_outw_bac
    global pool_fc_outc_bac
    feature_map_x = feature_map_y = 0
    kernel_size_x = kernel_size_y = 0
    ...
    with open(logpath, 'r') as file:
        for line in file:
            ...
            if line.startswith(f"layer_num:{layer_num}"):
                if "layer type:maxpool" in line or "layer type:avgpool" in line:
...
def inout_addr(N, C, H, W, div_size, layercnt):
    ...
    ADDRBLOCK += word_address
    fmain.write(f'%7s output start and end:{[start, ADDRBLOCK]}\n' % (f'layer{layercnt}'))
    layers_act_addr.append([f'layer{layercnt} output start and end:', [start, ADDRBLOCK]])
...
def netinout_addr(netpath):
    ...
    # net的输出层
    C, H, W, Cout = get_out_CHW(layer_cnts)
    fmain.write("netoutput_addr endof C:%3s   H:%3s   W:%3s\n" % (C, H, W))

    inout_addr(N, C, H, W, div_size, layer_cnts)
