...
class QuantOp():
    def __init__(self, model, bits=8, scale=1.0, threshold=0.01, quant_conv='mix-pow2'):
        self.bits = bits
        self.scale = scale
    ...
    def _getScalingFactor(self, module):
        ...
        module.register_buffer(f'scale', torch.tensor(best_scale))
        return best_scale
    ...
    def quantize8bit(self, **kwargs):
        with torch.no_grad():
            for module in self.weight.target_modules:
                module.weight.data = activation_quantization(module)[0]
